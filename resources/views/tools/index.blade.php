                     ...
                     x-data="{
                        ...
                        async addTool() {
                            if (!this.newTool.name.trim()) {
                                this.showNotification('New tool name cannot be empty.', 'error');
                                return;
                            }
                            if (this.tools.some(t => t.name.toLowerCase() === this.newTool.name.trim().toLowerCase())) {
                                this.showNotification('A tool with this name already exists.', 'error');
                                return;
                            }
                            try {
                                ...
                                if (!response.ok) {
                                    ...
                                    throw new Error(data.errors?.name?.[0] || 'Failed to add tool.');
                                }
                                const createdTool = await response.json();
                                this.tools.unshift(createdTool);
                                this.newTool = { name: '', comment: '' };
                                this.searchOpen = false;
                                this.showNotification('Tool added successfully.');
                            } catch (error) {
                        ...
                        async confirmDelete() {
                            try {
                                ...
                                this.showNotification('Tool deleted successfully.');
                            } catch (error) {
                                this.showNotification(error.message, 'error');
                            } finally {
                        ...
                        async toggleApproval(tool) {
                             try {
                                ...
                                this.showNotification('Approval status updated.');
                            } catch (error) {
                                this.showNotification(error.message, 'error');
                            }
                        }
                    ...
                    <div class="mb-6 p-4 border rounded-lg">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div class="relative" @click.away="searchOpen = false">
                                <input type="text" x-model.debounce.300ms="newTool.name" @focus="searchOpen = true" placeholder="Search or Create a Tool Name" class="w-full rounded-md shadow-sm border-gray-300" maxlength="50">
